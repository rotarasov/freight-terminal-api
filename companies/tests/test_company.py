...
class CreateNewCompanyAPITestCase(APITestCase):
    ...
    def test_valid_company_creation(self):
        response = self.client.post(reverse('companies:list'), self.valid_company, format='json')
    ...
    def test_invalid_company_creation(self):
        response = self.client.post(reverse('companies:list'), self.invalid_company, format='json')
...
class GetCompanyAPITestCase(APITestCase):
    ...
    def test_company_read(self):
        response = self.client.get(reverse('companies:detail', kwargs={'pk': self.company.account.id}))
...
class UpdateCompanyAPITestCase(APITestCase):
    ...
    def test_valid_company_partial_update(self):
        response = self.client.patch(
            reverse('companies:detail', kwargs={'pk': self.company.account.id}), self.valid_partial_data, format='json')
    ...
    def test_invalid_company_partial_update(self):
        response = self.client.patch(
            reverse('companies:detail', kwargs={'pk': self.company.account.id}), self.invalid_partial_data,
            format='json')
    ...
    def test_valid_company_update(self):
        response = self.client.put(
            reverse('companies:detail', kwargs={'pk': self.company.account.id}), self.valid_data, format='json')
        self.company = Company.objects.get(pk=self.company.account.id)
        serializer = CompanySerializer(self.company)
    ...
    def test_invalid_company_update(self):
        response = self.client.patch(
            reverse('companies:detail', kwargs={'pk': self.company.account.id}), self.invalid_data,
            format='json')
...
class DeleteCompanyAPITestCase(APITestCase):
    ...
    def test_valid_company_delete(self):
        response = self.client.delete(reverse('companies:detail', kwargs={'pk': self.company.account.id}),
                                      format='json')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_invalid_company_delete(self):
        response = self.client.delete(reverse('companies:detail', kwargs={'pk': self.not_existing_company_pk}),
                                      format='json')
